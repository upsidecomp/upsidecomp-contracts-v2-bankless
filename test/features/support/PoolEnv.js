...
function PoolEnv() {
  ...
  this.prizeStrategy = async function (wallet) {
    let prizeStrategy = await hardhat.ethers.getContractAt('BanklessMultipleWinnersHarness', this.env.prizeStrategy.address, wallet)
  ...
  this.addPrize = async function ({ user, tokenIds, name, symbol }) {
    ...
    const ERC721Mintable = await hre.ethers.getContractFactory("ERC721Mintable", wallet)
  ...
  this.currentPrizeTokenIdsOfIndex = async function ({ user, index, tokenIds }) {
      let wallet = await this.wallet(user)
      let prizeStrategy = await this.prizeStrategy(wallet)
      expect(await prizeStrategy.currentPrizeTokenIds(this.externalERC721Awards[index].address)).to.deep.equal(tokenIds.map((id) => BigNumber.from(id)))
  }

  this.expectUserToHaveExternalAwardToken = async function ({ user, index, tokenIds }) {
    let wallet = await this.wallet(user)
    for (var i=0; i < tokenIds.length; i++) {
        expect(await this.externalERC721Awards[index].ownerOf(tokenIds[i])).to.equal(wallet.address)
    }
  }

  this.expectEmptyPrizeList = async function () {
    expect(await this.env.prizeStrategy.currentPrizeAddresses(this.overrides)).to.deep.equal([])
  }
  ...
  this.addRandomPrize = async function () {
    ...
    await this.addPrize({ user: 0, tokenIds: tokenIds, name: "TEST", symbol: "TEST" })
  }
}
