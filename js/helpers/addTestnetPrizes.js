...
async function addTestnetPrizes(prizePoolContractAddress, numberOfPrizes) {
  ...
  let tx = await prizeContract.mintFastMany(prizePoolContractAddress, numberOfPrizes)
  ...
  const tokenIds = (createResultReceipt.logs.map(log => {
    try {
      l = erc721.parseLog(log)
      if (l.name == "Minted")
        return l.args.tokenId.toNumber()
    } catch (e) {
      return nulls
    }
